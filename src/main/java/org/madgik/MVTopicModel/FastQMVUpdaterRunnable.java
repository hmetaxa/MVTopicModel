                ...
                for (int x = 0; x < queues.size(); x++) {
                    ...
                    while ((delta = queues.get(x).poll()) != null) {
                        ...
                        if (delta.OldTopic != FastQMVParallelTopicModel.UNASSIGNED_TOPIC) {
                            ...
                            double p_wt = (currentTypeTopicCounts[delta.OldTopic] + beta[delta.Modality]) / (tokensPerTopic[delta.Modality][delta.OldTopic] + betaSum[delta.Modality]);

                            trees[delta.Modality][delta.Type].update(delta.OldTopic, (gamma[delta.Modality] * alpha[delta.Modality][delta.OldTopic] * p_wt));
                        }
                        double p_wt_newTopic = (currentTypeTopicCounts[delta.NewTopic] + beta[delta.Modality]) / (tokensPerTopic[delta.Modality][delta.NewTopic] + betaSum[delta.Modality]);
